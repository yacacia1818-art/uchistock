...
import { AddMemoSheet } from './AddMemoSheet';
...
// 買い物メモの確認・追加と、買ったものの記録（メモ経由/直接）をまとめた画面本体。
// Shopping.tsx（単独ページ）とRecords.tsx（記録タブ内）の両方から共通で使う
export function ShoppingMemoPanel() {
    ...
    <>
      <div className="card mb-16">
        <div className="card-head" style={{ marginBottom: 6 }}>
          <div className="section-title" style={{ marginBottom: 0 }}>買うものリスト</div>
          <button className="btn btn-outline btn-sm" onClick={() => setShowAddMemoOpen(true)}>
            <Plus size={14} /> 追加
          </button>
        </div>
        {memo.length === 0 ? (
